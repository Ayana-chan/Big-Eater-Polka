...
public class UI_BarLogic : MonoBehaviour
{
    /*
    ...
    private async void bloodUIAsync() {
        while (true) {
            await UniTask.Yield();
            if (isLoading) {
                continue;
    ...
    private async void loadingUIAsync() {
        while (true) {
            await UniTask.Yield();
            if (!isLoading) {
                continue;
    ...
    private async void valueDecreaseAsync() {    //control changingBloodImage
        while (true) {
            await UniTask.Yield();
            ...
            if (changingBloodImage.fillAmount <= bloodImage.fillAmount) {
                ...
                await UniTask.WaitUntil(() => changingBloodImage.fillAmount != bloodImage.fillAmount);
                //blood decrease
                if (changingBloodImage.fillAmount > bloodImage.fillAmount) {
                    await UniTask.Delay(System.TimeSpan.FromSeconds(delayOfChangingBloodDecrease), true);//delay to decrease
                }
            }
        }
    }

    private async void maxValueIncreaseAsync() {
        //float addMoreWidthOfBackLight = backLightRectTransform.rect.width - bloodRectTransform.rect.width* emptyBloodRectTransform.localScale.x;
        float addMoreWidthOfBackLight = 2 * (bloodRectTransform.position.x - backLightRectTransform.position.x);
        ...
        while (true) {
            ...
            //set backLight size
            aimWidthOfbackLight = emptyBloodRectTransform.rect.width * emptyBloodRectTransform.localScale.x   //blood actual width
            ...
            await UniTask.WaitUntil(()=> currentMaxValue != targetMaxValue);//wait for change
        }
    }
    ...
    private async void changeColorAsync() {
        ...
        while (true) {
            await UniTask.WaitUntil(()=> isLoading);
            bloodImage.sprite = loadBarTexture;
            await UniTask.WaitUntil(() => sliderProgress == 1);//load complete
    ...
    private async void textOnBarAsync() {
        //string randomChars = "@#$%&*";
        while (true) {
            await UniTask.Yield();
            if (isLoading) {
                ...
                if (loadingValueMode == 0) {
                    ...
                    if (bloodImage.fillAmount < 1) {
                        ...
                        barText.text = "100% - We beseech thee, ";
                    }
